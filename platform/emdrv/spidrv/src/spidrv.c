...
static void     WaitForTransferCompletionCS(SPIDRV_Handle_t handle);
 ...
 *    @n This function is blocking and returns when the transfer is complete,
 *    on timeout, or when @ref SPIDRV_AbortTransfer() is called.
 *    This is the same as SPIDRV_SReceiveB, but it will stop on CS going false
 *    and thus return the actual number of bytes/characters transferred on the
 *    bus. It will also return if the maximum number of bytes are read. Thus
 *    count is the buffer length, and rcount returns the actual number read
 *    into the buffer.
 *
 * @param[in]  handle Pointer to an SPI driver handle.
 *
 ...
 * @param[in]  count Number of bytes in transfer.
 *
 * @param[out] rcount Number of bytes actually read.
 *
 * @param[in]  timeoutMs Transfer timeout in milliseconds.
 *
 * @return
 ...
 *    returned.
 ******************************************************************************/
Ecode_t SPIDRV_SReceiveBL(SPIDRV_Handle_t handle,
                         void *buffer,
                         int  count,
						 int* rcount,
                         int  timeoutMs)
{
  ...
  WaitForTransferCompletionCS(handle);

  // set the actual length read
  *rcount = handle->transferCount-handle->remaining;
...
/***************************************************************************//**
 * @brief
 *    Start an SPI slave blocking transfer length limited.
 *
 * @note
 *    @n This function is blocking and returns when the transfer is complete,
 *    on timeout, or when @ref SPIDRV_AbortTransfer() is called.
 *    This is the same as SPIDRV_STransferB, but it will stop on CS going false
 *    and thus return the actual number of bytes/characters transferred on the
 *    bus. It will also return if the maximum number of bytes are read. Thus
 *    count is the buffer length, and rcount returns the actual number read
 *    into the buffer.
 ...
 *    returned.
 ******************************************************************************/
Ecode_t SPIDRV_STransferBL(SPIDRV_Handle_t handle,
                          const void *txBuffer,
                          void *rxBuffer,
                          int buflen,
			  int* rcount,
                          int timeoutMs)
{
  ...
  if ( (retVal = TransferApiBlockingPrologue(handle, (void*)txBuffer, buflen))
       != ECODE_EMDRV_SPIDRV_OK ) {
    return retVal;
  }
  ...
  StartTransferDMA(handle, txBuffer, rxBuffer, buflen, BlockingComplete);
 ...
 * @brief Wait for transfer completion.
 *
 * Same as WaitForTransferCompletion except that CS going false will also
...
#error "No valid SPIDRV DMA engine defined."
#endif
      // CS not active, terminate
      if (GPIO_PinInGet((GPIO_Port_TypeDef)handle->csPort, handle->csPin)) {

    	  // refresh remainder count
    	  CORE_ENTER_ATOMIC();
    	  DMADRV_TransferRemainingCount(handle->rxDMACh, &handle->remaining);
    	  CORE_EXIT_ATOMIC();
    	  if (handle->transferCount != handle->remaining)
    	      SPIDRV_AbortTransfer(handle);
